...
def event_time_to_string(start, end):
    if start["dateTime"]:
        return "from {} to {}".format(
            parse_datetime(start["dateTime"]), parse_time(end["dateTime"])
        )
    else:
...
async def on_event_match(message, event):
    ...
    if location_set:
        ...
        await client.send_message(
            message.channel,
            "Ok, set {} to location {}".format(event["summary"], room_no),
        )
    else:
...
async def on_message(message):
    ...
    if client.user.mentioned_in(message):
        if (
                ...
                if words_in(["plan", "event", "run", "schedule"], message.content):
                    if len(events) > 0:
                        ...
                        if len(upcoming) > 0:
                            ...
                            messages.append(
                                "We don't actually have any events completely ready to publish yet"
                            )
                        ...
                        if len(no_time) > 0:
                            messages.append(
                                "These ones we haven't got a specific time for: \n {}".format(
                                    no_time
                                )
                            )
                        ...
                        if len(no_description) > 0:
                            messages.append(
                                "These ones don't have a description: \n {}".format(
                                    no_description
                                )
                            )
                        ...
                        if len(no_description) > 0:
                            messages.append(
                                "These ones don't have a location: \n {}".format(
                                    no_location
                                )
                            )
                        await client.send_message(
                    ...
                    await client.send_typing(message.channel)
                    events = get_upcoming(
                        "rmit.edu.au_cqqvahbep63tttoc551ug04q88@group.calendar.google.com"
                    )
                    if len(events) > 0:
                        upcoming = "\n".join(
                            [event_to_string(event) for event in events]
                        )
                        counter = 0
                        await client.send_message(
                            message.channel,
                            content="Here are the meetings we have planned: \n {}".format(
                                upcoming
                            ),
                        )
                    else:
                        await client.send_message(
                            message.channel, "No meetings coming up"
                        )
                else:
                    await client.send_message(
                        message.channel,
                        content="Hey {}, I'm Stacy, Assistant here at Bots Unlimited. I can tell you about meetings and events that are planned but not published".format(
                            name
                        ),
                    )
        else:
            await client.send_message(
                message.channel,
                content="Hey {}, I'm Stacy, Assistant here at Bots Unlimited. I'm only available to executive".format(
                    name
                ),
            )
