...
async def print_calendar(events, event_type, channel):
    if len(events) > 0:
        ...
        await client.send_message(
            channel,
            content="Here are the {} we have planned: \n {}".format(
                event_type, upcoming
            ),
        )
    else:
...
async def on_message(message):
    ...
    if client.user.mentioned_in(message):
        if (
            ...
            await client.send_message(
                message.channel,
                "Hi {}, I'm Brent, Operations Officer here at Bots Unlimited. I can publish Eventbrite events".format(
                    name
                ),
            )
        else:
            await client.send_message(
                message.channel,
                "Hi {}, I'm Brent, Operations Officer here at Bots Unlimited. I do operations work for executive".format(
                    name
                ),
            )
